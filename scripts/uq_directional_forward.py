r"""One forward VAF run at θ_MAP + n·δθ_1σ (linearization check).

Replicates run_forward.py's prognostic setup exactly (same model, friction,
melt, BCs, fixed A/C, 3D VAF integral) but UN-taped, with controls perturbed
along the QoI posterior-uncertainty direction from uq_make_direction.py.

If the Isaac et al. linearization holds, ΔVAF(n) ≈ n·σ_post (a straight line).

Run:
    OMP_NUM_THREADS=4 python uq_directional_forward.py --n 1.0
"""
import argparse
import numpy as np
...
DIR_FILE = DATA / "results" / "uq_linearity_direction.npz"
...
def main():
    ...
    ap.add_argument("--n", type=float, required=True, help="amplitude (× δθ_1σ)")
    ap.add_argument("--T", type=float, default=50.0)
    args = ap.parse_args()
    n_amp, T_YEARS = args.n, args.T
    num_steps = max(1, int(round(T_YEARS / DT)))
    nstr = f"{n_amp:+.2f}".replace("+", "p").replace("-", "m").replace(".", "_")
    out_file = DATA / "results" / f"uq_linearity_n{nstr}.npz"
    print(f"=== linearity forward: n={n_amp:+.2f}, T={T_YEARS} yr, {num_steps} steps ===", flush=True)
    ...
    d = np.load(str(DIR_FILE))
    dA, dC = d["dA"], d["dC"]
    sigma_post_Gt = float(d["sigma_post_Gt"])
    ...
    θ_A.dat.data[:] = θA_map.dat.data_ro + n_amp * dA
    θ_C.dat.data[:] = θC_map.dat.data_ro + n_amp * dC
    ...
    try:
        ...
        np.savez(str(out_file), n=n_amp, failed=True, sigma_post_Gt=sigma_post_Gt)
        return
    ...
    for step in range(1, num_steps + 1):
        t = step * DT
        try:
            # Recinos et al. (2023) Eq. 5 depth-dependent sub-shelf melt (see run_forward.py)
        ...
        if step % max(1, num_steps // 10) == 0 or step == num_steps:
            print(f"  t={t:5.1f}: VAF={vaf_ts[-1]:.1f} (ΔVAF={vaf_ts[-1]-vaf0:+.2f}), {n_fail} fail", flush=True)

    dVAF = vaf_ts[-1] - vaf0
    print(f"  DONE n={n_amp:+.2f}: ΔVAF={dVAF:+.3f} Gt | linearized n·σ_post={n_amp*sigma_post_Gt:+.3f} Gt | {n_fail} fails", flush=True)
    np.savez(str(out_file), n=n_amp, failed=False, time=np.array(times), vaf_Gt=np.array(vaf_ts),
             dVAF=dVAF, vaf0=vaf0, sigma_post_Gt=sigma_post_Gt, lin_pred=n_amp * sigma_post_Gt, n_fail=n_fail)
