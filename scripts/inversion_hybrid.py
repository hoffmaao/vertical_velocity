...
SSA_FILE = DATA_DIR / "mesh" / "inversion_ssa.h5"
OUTPUT_FILE = DATA_DIR / "mesh" / "inversion_hybrid.h5"
...
def main():
    ...
    # ── 3. Controls (warm-start from SSA) ──
    ...
    # Grounded mask
    ϕ_mask = Function(Q_2d).interpolate(
        1 - ρ_W * g * max_value(0, h_2d - s_2d) / (ρ_I * g * h_2d))
    ...
    # Warm-start from SSA result
    if SSA_FILE.exists():
        with firedrake.CheckpointFile(str(SSA_FILE), "r") as chk:
            mesh_ssa = chk.load_mesh()
            θ_A_raw = chk.load_function(mesh_ssa, "log_fluidity").dat.data_ro.copy()
            θ_C_raw = chk.load_function(mesh_ssa, "log_friction").dat.data_ro.copy()
            # Zero θ_C on floating ice and clip both to physical range
            θ_C_raw[grounded_mask.dat.data < 0.5] = 0.0
            θ_A.dat.data[:] = np.clip(θ_A_raw, -10, 10)
            θ_C.dat.data[:] = np.clip(θ_C_raw, -10, 10)
            u_ssa = chk.load_function(mesh_ssa, "velocity")
        # Use SSA velocity as initial guess (lifted to 3D)
        u0_ssa = icepack.utilities.lift3d(u_ssa, V_lift)
        u0.project(u0_ssa)
        print(f"   SSA warm start: "
    ...
    A_test = icepack.utilities.lift3d(
        Function(Q_2d).interpolate(A_0 * exp(θ_A)), Q_lift)
    C_test = icepack.utilities.lift3d(
        Function(Q_2d).interpolate(C_0 * exp(θ_C * grounded_mask)), Q_lift)
    ...
    def simulation(controls):
        ...
        A = icepack.utilities.lift3d(
            Function(Q_2d).interpolate(A_0 * exp(θ_A_c)), Q_lift)
        C = icepack.utilities.lift3d(
            Function(Q_2d).interpolate(C_0 * exp(θ_C_c * grounded_mask)), Q_lift)
    ...
    def loss_functional(u):
        # Extract surface velocity (at ζ=1) from the hybrid model
        u_surface = compute_surface_velocity(u)
        u_interp = Function(Δ).interpolate(u_surface[0])
        v_interp = Function(Δ).interpolate(u_surface[1])
        δu, δv = u_interp - u_o, v_interp - v_o
        return 0.5 / Constant(float(N_obs)) * ((δu / σ_x)**2 + (δv / σ_y)**2) * dx
    ...
    A_opt = icepack.utilities.lift3d(
        Function(Q_2d).interpolate(A_0 * exp(θ_A_opt)), Q_lift)
    C_opt = icepack.utilities.lift3d(
        Function(Q_2d).interpolate(C_0 * exp(θ_C_opt * grounded_mask)), Q_lift)
    ...
    fields = [
        ...
        (θ_A_opt, "θ_A (log-fluidity)", "RdBu_r", -5, 5),
        (θ_C_opt, "θ_C (log-friction)", "RdBu_r", -5, 5),
    ...
    fig.tight_layout()
    fig.savefig(str(DATA_DIR / "figures" / "inversion_hybrid.png"), dpi=200)
